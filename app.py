...
def validate_upload_extension(filename: str, allowed_extensions: set[str]) -> bool:
    return bool(filename) and Path(filename).suffix.lower() in allowed_extensions

def validate_uploaded_file(file, allowed_extensions: set[str], label: str) -> tuple[bool, str]:
    if not file or not file.filename:
        return False, f"Selecione {label}."
    if not validate_upload_extension(file.filename, allowed_extensions):
        formatos = ", ".join(sorted(allowed_extensions))
        return False, f"Formato inválido para {label}. Use apenas: {formatos}."
    return True, ""
...
def is_logged_in() -> bool:
    return bool(session.get("authenticated")) or not auth_is_configured()
...
def login():
    if not auth_is_configured():
        flash("Login não está ativado. Configure ADMIN_USERNAME e ADMIN_PASSWORD no Render para proteger o sistema.", "error")
...
def logout():
    ...
    return redirect(url_for("login" if auth_is_configured() else "home"))
...
def arquivo_muito_grande(_error):
    flash("Arquivo muito grande. Reduza o tamanho ou divida o processamento em lotes menores.")
    return redirect(request.referrer or url_for("home"))

@app.errorhandler(500)
def erro_interno(_error):
    logger.exception("Erro interno não tratado")
    flash("Ocorreu um erro interno. Tente novamente e, se continuar, verifique os logs do Render.")
    return redirect(request.referrer or url_for("home"))
